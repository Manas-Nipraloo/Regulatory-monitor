...
async def discover_rbi_fema_master_circulars(run_date: date, site: SiteConfig) -> list[DiscoveredArticle]:
    ...
    for row in soup.find_all("tr"):
        ...
        pdf_url = _first_pdf_link(row, site.url)
        if not pdf_url:
            continue

        title, source_url = _first_detail_link(row, site.url) or (_clean_title(row_text), site.url)
        articles.append(
            DiscoveredArticle(
                site=site,
                title=title,
...
def _first_detail_link(row: object, base_url: str) -> tuple[str, str] | None:
    for link in row.find_all("a", href=True):
        ...
        if title and "notificationuser.aspx" in href.casefold():
            return _clean_title(title), href
    return None
